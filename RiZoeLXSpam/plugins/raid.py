...
async def spam(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(RiZoeL) == 2:
            ...
            g = a.id
            if int(g) in RiZoeLX:
                text = f"I can't raid on @LUXCLUB_SERGIO 's Owner"
                await e.reply(text, parse_mode=None, link_preview=None )
            elif int(g) == OWNER_ID:
                text = f"This guy is Master of the Bot."
                await e.reply(text, parse_mode=None, link_preview=None )
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(e.text) > 11:
            ...
            if int(user_id) in RiZoeLX:
                text = f" can't raid on @LUXCLUB_SERGIO 's Owner."
                await e.reply(text, parse_mode=None, link_preview=None )
            elif int(user_id) == OWNER_ID:
                text = f"This guy is a Master of the Bot."            
